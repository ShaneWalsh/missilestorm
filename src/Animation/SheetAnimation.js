...
export default class SheetAnimation extends Animation{
        
    update({constLoop=false}){
		let  p = this.props;
		if(p.angle != 0){
			DrawRotateImage(  p.startx,p.starty,p.sizex,p.sizey,    p.x,p.y,p.sizex,p.sizey,   p.ctxDraw,p.sheet,p.angle,p.x+p.hWidth, p.y+p.hHeight);
		}
		else{
	...
	setSheet({sheet,startx,starty,sizex,sizey,movex,movey,steps}){ 
		let  p = this.props;
		p.sheet = sheet; // img
		p.startx = startx;
		p.starty = starty;
