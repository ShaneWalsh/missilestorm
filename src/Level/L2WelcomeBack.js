...
export default class L2WelcomeBack extends LevelTemplate{
	...
	*/
	update({}){ // this is where we can do the condition checking for the level progression
        ...
        if(gameInstance.props.ship.props.health < 0 || (p.tower1.props.health < 1 && p.tower2.props.health < 1 && p.tower3.props.health < 1 && p.tower4.props.health < 1)){
	...
	levelUpEffect(topic, {botCount}){ // we have moved up a level, what does this mean for the game?
        ...
        let dasherStats = {speed:1,health:4, damage:5};
        let gliderStats = {lazerTimer:200};

        p.waves--;
        if(level == 1){ 
            p.botslevel = {spawnThreshold:1,dashersMax: 20, glidersMax:5, keepersMax: 0, carriersMax: 0, cometsMax: 0, dasherStats,gliderStats};
            gameInstance.props.botManager.updateBots(p.botslevel);
        }else if(level == 2){
            p.botslevel = {spawnThreshold:1,dashersMax: 20, glidersMax:5, keepersMax: 1, carriersMax: 0, cometsMax: 0, dasherStats,gliderStats};
            gameInstance.props.botManager.updateBots(p.botslevel);
        }else if(level == 3){
            p.botslevel = {spawnThreshold:1,dashersMax: 25, glidersMax:5, keepersMax: 0, carriersMax: 0, cometsMax: 0, dasherStats,gliderStats};
            gameInstance.props.botManager.updateBots(p.botslevel);
        }else if(level == 4){
            dasherStats = {speed:1,health:8, damage:5};
            gliderStats = {health:8, lazerTimer:120};
            p.botslevel = {spawnThreshold:1,dashersMax: 25, glidersMax:5, keepersMax: 1, carriersMax: 0, cometsMax: 0, dasherStats,gliderStats};
            gameInstance.props.botManager.updateBots(p.botslevel);
        }else if(level == 5){
            dasherStats = {speed:1,health:8, damage:5};
            gliderStats = {health:8, lazerTimer:120};
    ...
    init({}){
        ...
        //botManager
        p.botslevel = {spawnThreshold:1,dashersMax: 25, glidersMax:10, keepersMax: 0, carriersMax: 0, cometsMax: 0, dasherStats,gliderStats};
        gameInstance.props.botManager.updateBots(p.botslevel);
